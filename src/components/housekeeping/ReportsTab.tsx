        ...
        <ModalShell kicker={`Report · ${month}`} title={detail.name} subtitle={`${detail.tasksCompleted} tasks · ${detail.shiftHours} shift hours`} onClose={() => setDetail(null)} size="lg">
          <div className="space-y-4 p-5">
            ...
            <table className="w-full text-left text-sm">
              <thead><tr className="border-b text-xs uppercase tracking-wider text-muted-foreground"><th className="py-2">Task</th><th>Started</th><th>Finished</th></tr></thead>
              <tbody className="divide-y">
                {(detail.tasks ?? []).map((t) => <tr key={t.id}><td className="py-2"><span className="font-medium">{taskName(t)}</span> <span className="text-xs text-muted-foreground">{t.taskNo}</span></td><td>{fmtDateTime(t.startedAt)}</td><td>{fmtDateTime(t.completedAt)}</td></tr>)}
