...
export default function ShiftSummaryModal({
  ...
  const rejected = live.status === 'REJECTED_START' || live.status === 'REJECTED_END'
  const statusLabel = rejected ? 'Rejected' : live.status === 'ENDED' ? 'Cleared' : live.status === 'REQUESTED_END' ? 'Awaiting review' : live.status === 'ACTIVE' ? 'In progress' : 'Pending'
  const statusTone = rejected ? 'bg-destructive text-white' : live.status === 'ENDED' ? 'bg-success text-white' : 'bg-accent text-accent-foreground'
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/60 px-4 py-8">
      <div className="w-full max-w-4xl border-2 border-foreground/25 bg-card shadow-[8px_8px_0_0_rgba(0,0,0,0.25)]">
        ...
        <div className="grid grid-cols-2 border-b bg-card text-sm sm:grid-cols-4">
          ...
          <MetaCell label="Duration"><span className="font-mono font-bold tabular-nums">{summary.hours.toFixed(2)} h</span></MetaCell>
        </div>

        <div className="space-y-5 p-5">
          {rejected && (
            <div className="border-2 border-destructive/50 bg-destructive/10 p-3">
              <p className="text-xs font-bold uppercase tracking-wider text-destructive">Rejected</p>
          ...
          {decidedVariance != null && Math.abs(decidedVariance) > 0.005 && (
            <div className={cn('border-2 p-3 text-sm font-bold', decidedVariance > 0 ? 'border-success/50 bg-success/10 text-success' : 'border-destructive/50 bg-destructive/10 text-destructive')}>
          ...
          <ShiftSummaryTable title="Transactions" empty="No transactions recorded." columns={['Reference', 'Method', 'Time', 'Amount']} right={[3]}>
            {summary.transactions.map((t) => (
              <tr key={t.id}>
                ...
                <td className="px-3 py-2 tabular-nums">{new Date(t.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</td>
                <td className={cn('px-3 py-2 text-right font-bold tabular-nums', t.direction === 'IN' ? 'text-success' : 'text-destructive')}>{t.direction === 'IN' ? '+' : '-'}{formatKes(t.amount)}</td>
              </tr>
            ))}
          </ShiftSummaryTable>
        ...
        {approval && (onApprove || onReject) && (
          <div className="border-t-2 border-foreground/15 bg-muted/40 p-5">
            ...
            <div className="mt-2 grid gap-2 sm:grid-cols-2">
              ...
              <button type="button" onClick={() => setSomethingOff(true)} className={cn('border-2 p-3 text-left text-sm transition', somethingOff ? 'border-destructive bg-destructive/10' : 'bg-card hover:bg-muted')}>
                <span className="font-bold">Something is off</span>
            ...
            {somethingOff && (
              <div className="mt-3 space-y-3">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-muted-foreground">Amount off (KSh) — positive if over, negative if short</label>
                  <div className="mt-1.5 flex flex-wrap items-center gap-2">
                    <input type="number" step="0.01" value={cashInput} onChange={(e) => setCashInput(e.target.value)} placeholder="e.g. -500" className="w-40 border-2 bg-card px-3 py-2 text-sm" />
                    {cashBucket && <span className="text-xs text-muted-foreground">System cash total: {formatKes(cashBucket.total)}</span>}
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-muted-foreground">Notes — what happened</label>
                  <textarea value={noteInput} onChange={(e) => setNoteInput(e.target.value)} maxLength={500} className="mt-1.5 min-h-20 w-full border-2 bg-card px-3 py-2 text-sm" placeholder="Explain the discrepancy" />
                </div>
              </div>
            )}
            <div className="mt-4 flex flex-wrap justify-end gap-2">
              {onReject && <button disabled={flaggedMissingInfo || busyKey === `${session.id}:reject-end`} onClick={() => onReject(decision)} className="inline-flex items-center gap-2 border-2 border-destructive px-4 py-2 text-sm font-bold uppercase tracking-wide text-destructive transition hover:bg-destructive/10 disabled:opacity-60">{busyKey === `${session.id}:reject-end` && <LuLoaderCircle className="size-4 animate-spin" />}Reject</button>}
              {onApprove && <button disabled={flaggedMissingInfo || busyKey === `${session.id}:approve-end`} onClick={() => onApprove(decision)} className={cn('inline-flex items-center gap-2 px-4 py-2 text-sm font-bold uppercase tracking-wide text-white transition hover:brightness-110 disabled:opacity-60', somethingOff ? 'bg-destructive' : 'bg-success')}>{busyKey === `${session.id}:approve-end` && <LuLoaderCircle className="size-4 animate-spin" />}{somethingOff ? 'End shift with discrepancy' : 'Mark cleared and end shift'}</button>}
            </div>
...
function ShiftMiniStat({ label, value, hint, bar }: { label: string; value: string; hint?: string; bar: string }) {
  return (
...
function ShiftSummaryTable({ title, empty, columns, right, children }: { title: string; empty: string; columns: string[]; right: number[]; children: ReactNode[] }) {
  return (
    <div className="overflow-hidden border">
      <div className="flex items-center justify-between border-l-4 border-accent bg-muted px-3 py-2">
        <span className="text-xs font-bold uppercase tracking-wider">{title}</span>
        <span className="text-[11px] font-bold tabular-nums text-muted-foreground">{children.length}</span>
      </div>
      {children.length === 0 ? <p className="p-3 text-sm text-muted-foreground">{empty}</p> : (
        <div className="max-h-56 overflow-y-auto">
          <table className="w-full text-left text-sm">
            <thead className="sticky top-0 bg-primary text-primary-foreground">
              <tr>{columns.map((c, i) => <th key={c} className={cn('px-3 py-2 text-[11px] font-bold uppercase tracking-wider', right.includes(i) && 'text-right')}>{c}</th>)}</tr>
            </thead>
            <tbody className="divide-y [&>tr:nth-child(even)]:bg-muted/30">{children}</tbody>
          </table>
        </div>
      )}
    </div>
    ...
    <div className="space-y-3 border bg-muted/20 p-4">
      ...
      <div className="flex flex-wrap gap-2">
        {([['ENDED', 'Cleared'], ['REJECTED_END', 'Rejected']] as const).map(([value, label]) => (
          <button key={value} type="button" onClick={() => setStatus(value)} className={cn(' border px-3 py-1.5 text-sm font-semibold', status === value ? (value === 'ENDED' ? 'border-success bg-success/10' : 'border-destructive bg-destructive/10') : 'hover:bg-muted')}>{label}</button>
        ))}
      </div>
      <div>
        ...
        <input type="number" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} className="mt-1.5 w-40 border px-3 py-2 text-sm" />
      </div>
      {hasAmount && (
        <div>
          <label className="block text-xs font-semibold uppercase tracking-wide text-muted-foreground">Notes — what happened</label>
          <textarea value={note} onChange={(e) => setNote(e.target.value)} maxLength={500} className="mt-1.5 min-h-16 w-full border px-3 py-2 text-sm" />
        </div>
      )}
      <div>
        ...
        <input value={reason} onChange={(e) => setReason(e.target.value)} maxLength={500} className="mt-1.5 w-full border px-3 py-2 text-sm" placeholder="Why is the outcome being changed?" />
      </div>
      <div className="flex justify-end gap-2">
        <button type="button" onClick={() => setOpen(false)} className=" border px-3 py-1.5 text-sm font-semibold hover:bg-muted">Cancel</button>
        <button type="button" disabled={invalid || saving} onClick={() => void save()} className="inline-flex items-center gap-2 bg-primary px-4 py-1.5 text-sm font-semibold text-primary-foreground disabled:opacity-60">{saving && <LuLoaderCircle className="size-4 animate-spin" />}Save</button>
      </div>
    </div>
  )
}
      ...
      {locked ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div>
            ...
            <select value={type} onChange={(e) => setType(e.target.value as 'ALLOWANCE' | 'DEDUCTION')} className="mt-1.5 w-full border px-3 py-2 text-sm">
          ...
          <div>
            ...
            <input type="number" min="0" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} className="mt-1.5 w-full border px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wide text-muted-foreground">Salary month</label>
            <input type="month" value={month} onChange={(e) => setMonth(e.target.value)} disabled={recorded != null} className="mt-1.5 w-full border px-3 py-2 text-sm disabled:opacity-60" />
          </div>
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wide text-muted-foreground">Label (optional)</label>
            <input value={label} onChange={(e) => setLabel(e.target.value)} maxLength={120} className="mt-1.5 w-full border px-3 py-2 text-sm" placeholder="Shift shortage - 10 March" />
          </div>
          ...
          <div className="flex justify-end gap-2 sm:col-span-2">
            <button type="button" onClick={() => setOpen(false)} className=" border px-3 py-1.5 text-sm font-semibold hover:bg-muted">Cancel</button>
            <button type="button" disabled={saving || !(Number(amount) > 0)} onClick={() => void save()} className="inline-flex items-center gap-2 bg-primary px-4 py-1.5 text-sm font-semibold text-primary-foreground disabled:opacity-60">{saving && <LuLoaderCircle className="size-4 animate-spin" />}{recorded ? 'Update' : 'Record'}</button>
          </div>
        </div>
      )}
    </div>
